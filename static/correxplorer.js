...
var get_elements_size = function(){
  var tooltip = body.select('div#corrs');

  var container = body.select('div#corrs-container');

  // getting windows size
  var w = window,
      d = document,
      e = d.documentElement,
      bod = d.getElementsByTagName('body')[0],
      x = w.innerWidth || e.clientWidth || bod.clientWidth,
      y = w.innerHeight|| e.clientHeight|| bod.clientHeight;
      return [x,y];
}
...
var main = function(corr, label_col, label_row){
  ...
  var svg = tooltip.append('svg')
    ...
    .attr('height', y);
  // setting container
  var container = $('#corrs-container');
    container.animate({'height':1.5*y,"margin-left":"50px"});
